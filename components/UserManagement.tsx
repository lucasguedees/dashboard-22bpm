...
import { supabaseReady, listAppUsers, createAppUser, deleteAppUser, updateAppUser, updateUserPassword, updateAppUserAdmin, AppUserRow } from '@/lib/api';
...
const UserManagement: React.FC = () => {
  ...
  const [resettingPasswordFor, setResettingPasswordFor] =
  useState<AppUserRow | null>(null);
  ...
  const handleResetPassword = async () => {
    ...
    try {
      await updateUserPassword(resettingPasswordFor.auth_user_id, newPassword);
  ...
  const startEditUser = (user: LocalUser) => {
    setEditingUser(user.auth_user_id);
    setFormData({
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    if (supabaseReady) {
      try {
        const updated = await updateAppUserAdmin(editingUser, {
          username: formData.username,
          email: formData.email,
          role: formData.role,
          rank: formData.rank,
          ...
          <tbody className="divide-y divide-gray-800">
            {users.map(u => (
              <tr key={u.id} className="hover:bg-gray-800/20 transition-colors">
                ...
                <td className="px-6 py-4">
                  <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${u.role === 'ADMIN' ? 'bg-amber-500/10 text-amber-500 border border-amber-500/20' :
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center justify-end space-x-2">
                    ...
                    <button
                      onClick={() => setResettingPasswordFor({

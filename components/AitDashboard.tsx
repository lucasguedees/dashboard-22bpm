...
const AitDashboard: React.FC<AitDashboardProps> = ({ data, isAdmin, onDelete, onEdit, userGroup, userCity }) => {
  ...
  // Carregar cidades ativas do banco de dados
  useEffect(() => {
    const loadActiveCities = async () => {
      try {
        ...
        if (cities.length > 0) {
          ...
          // Se houver uma cidade de usuário e ela estiver nas cidades ativas, selecioná-la
  ...
  const selectGroupCities = () => {
    if (userGroup && CITY_GROUPS[userGroup as CityGroup]) {
      const groupCities = CITY_GROUPS[userGroup as CityGroup].filter(city => 
        activeCities.includes(city)
      );
      setSelectedCities(groupCities);
    }
  };

  // Função para limpar todas as seleções e voltar para a cidade do usuário
  const clearSelection = () => {
    if (userCity && activeCities.includes(userCity)) {
      setSelectedCities([userCity]);
    } else if (activeCities.length > 0) {
      setSelectedCities([activeCities[0]]);
    } else {
      setSelectedCities([]);
    }
  };

  // Usar todas as cidades ativas
  ...
  // Inicializar com a cidade do usuário ou a primeira cidade ativa por padrão
  useEffect(() => {

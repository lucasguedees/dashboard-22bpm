...
const AitForm: React.FC<AitFormProps> = ({ onSave, onCancel, initialData }) => {
  ...
  // O formulário trabalha com meses 1-12, mas o banco de dados espera 0-11
  const [formData, setFormData] = useState({
    city: initialData?.city || CITIES[0],
    // Já que o formulário trabalha com 1-12, não precisamos converter aqui
    month: initialData !== undefined ? initialData.month + 1 : new Date().getMonth() + 1,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Converter do formato do formulário (1-12) para o formato do banco (0-11)
    const monthForDb = formData.month - 1;
    
    // Criar o objeto com os dados para salvar
    const dataToSave = {
      city: formData.city,
      month: monthForDb, // Já convertido para 0-11
      year: formData.year,
          ...
          <div className="flex flex-col">
            ...
            >
              {MONTHS.map((month, idx) => {
                // idx é 0-11, mas o valor do option é 1-12
                return (
            ...
            {/* Debug: Mostrar o valor atual do mês */}
            <input type="hidden" value={formData.month} />
          </div>
